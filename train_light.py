...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        epoch_start = time.time() #此epoch开始时间
        model.train()
        train_loss = 0.0
        #训练
        loss_type = change_loss(epoch,loss_type)
        ...
        f.write("{:0>3} epochs, Loss error: {:.6f}, Rot error:{:.4f}, Trans error:{:.4f}, lr={:.6f}\n".format(epoch, val_loss/len(val_loader)  ,rotation_loss/len(ValidationData) ,  trans_loss*100 / len(ValidationData),scheduler.get_lr()[0] ))
        f.close()
